...
class Country extends \Illuminate\Database\Eloquent\Model {
	...
	public function name() {
		$place = Place::where('country_id', '=', $this->geoname_id)->first();
		if (!$place) return '';
		return $place->name;
	}
	...
	static public function byName($name) {
		...
		if (!$place) {
			// Handles cases like "Korea, Republic of" / "Republic of Korea"
			$s = explode(',', $name);
			if (count($s) == 2) $place = Place::where('country_id', '>', 0)->where('name', '=', trim($s[1]) . ' ' . trim($s[0]))->first();
		}
		if (!$place) throw new \Exception('Invalid name: ' . $name);
		return $place->country();
	}

	static public function byCode($code) {
